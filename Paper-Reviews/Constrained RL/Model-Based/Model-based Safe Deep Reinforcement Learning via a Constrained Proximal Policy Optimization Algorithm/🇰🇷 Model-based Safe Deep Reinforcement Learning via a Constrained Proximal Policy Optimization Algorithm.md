...
[[📚 Constrained Policy Optimization]]은 trust region based constrained optimization 프레임워크를 제안, surrogate functions(objective와 constraint에 관련된)을 사용하여 근사한 뒤 projection 단계를 통해 제약 조건 위반을 조정

- Model-based

[[📚 Safe Reinforcement Learning by Imagining the Near Future]]에서는 cost function을 사용하는 대신, reward functions을 통해 페널티를 주는 방식으로 환경의 모델을 학습하여 SAC 알고리즘으로 정책을 최적화.


## 3 Methods

### 3.1 Model-based Constrained RL
...
### 3.2 Challenges in Environment Model Learning

#### 3.2.1 Handling aleatoric and epistemic uncertainty
...
#### 3.2.3 Implication of using truncated horizon in Constrained RL
...
[[🇰🇷 Safe Model-Based Reinforcement Learning with Robust Cross-Entropy Method]]에서처럼 pseudo-lidar 대신 상대 좌표를 observation으로 사용 

![image](imgs/algorithm.png)

실제 환경과의 상호작용 없이 정책을 정확하게 평가하는 것은 어려움

Ensemble model의 Performance Ratio (PR)를 측정 ([[📚 Model-ensemble trust-region policy optimization]]에서 제안한 방법)
...
[[🇰🇷 Safe Model-Based Reinforcement Learning with Robust Cross-Entropy Method]] 이 논문에서 사용한 Safety Gym 환경에서 테스트를 했는데, 왜 model-based 비교군으로는 다른 방법 [safe-LOOP](https://arxiv.org/abs/2008.10066)로만 비교했는지? 둘 다 2021년에 나온 논문인데? 성능 때문인가?
