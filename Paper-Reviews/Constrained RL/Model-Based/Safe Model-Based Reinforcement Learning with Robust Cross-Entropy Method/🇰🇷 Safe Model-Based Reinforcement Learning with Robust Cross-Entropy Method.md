...
\tag{1}
$$
식 (1)에서 $\arg\max$ 왼쪽에 있는 $\mathcal{X}$는 optimal action sequence를 의미

**Robust Cross-Entropy Method**

위 최적화 문제를 풀기 위해 [[🇰🇷 Deep Reinforcement Learning in a Handful of Trials using Probabilistic Dynamics Models]]에서 사용한 trajectory sampling (TS)을 사용. (샘플링된 미래 trajectories 중 가장 제약 조건 위반을 위반한 최악의 경우를 고려)
...
3. feasible set으로부터 제약 조건을 만족하는 해(식 (2))의 집합을 선택하고, 정렬하여 상위 $k$개의 샘플로 distribution parameter를 업데이트
